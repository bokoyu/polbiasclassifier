...
def evaluate_bias_model(data_path, batch_size=8):
    print("Loading dataset for RoBERTa bias model evaluation...")
    df = load_data(data_path)
    
    X_val, y_val = df['text'], df['label']

    tokenizer = RobertaTokenizer.from_pretrained("savedmodels/bias_model")
    model = RobertaForSequenceClassification.from_pretrained("savedmodels/bias_model")
